<div align="center">
  <img src="assets/Audic-new.png" alt="Audic Music Logo" width="120"/>

  <h1>ðŸŽµ Audic Music</h1>

  <p><strong>Your music, your way â€” ad-free, beautifully crafted, and built for Android.</strong></p>

  <p>
    <a href="#-features">
      <img src="https://img.shields.io/badge/features-ðŸŽ¯-8A2BE2?style=flat-square" alt="Features"/>
    </a>
    <a href="#-download">
      <img src="https://img.shields.io/badge/download-ðŸ“¥-00C853?style=flat-square" alt="Download"/>
    </a>
    <a href="LICENSE">
      <img src="https://img.shields.io/badge/license-GPLv3-blue?style=flat-square" alt="License"/>
    </a>
  </p>
</div>

---

## âœ¨ Overview

Audic Music is a **modern, ad-free music player for Android** that puts you in control. Stream millions of songs without interruptions, sing along with real-time synced lyrics, discover what's playing around you with a tap, and enjoy personalized AI-powered recommendations â€” all offline-friendly and privacy-respecting.

No subscriptions. No ads. Just pure music.

---

## ðŸ“¸ Screenshots

<div align="center">
  <table>
    <tr>
      <td align="center"><strong>ðŸ  Home</strong></td>
      <td align="center"><strong>â–¶ï¸ Player</strong></td>
      <td align="center"><strong>ðŸŽ¤ Lyrics</strong></td>
    </tr>
    <tr>
      <td><img src="Screenshots/sc_1.png" width="200" style="border-radius: 12px;"/></td>
      <td><img src="Screenshots/sc_2.png" width="200" style="border-radius: 12px;"/></td>
      <td><img src="Screenshots/sc_3.png" width="200" style="border-radius: 12px;"/></td>
    </tr>
    <tr>
      <td align="center"><strong>ðŸ” Explore</strong></td>
      <td align="center"><strong>ðŸ“š Library</strong></td>
      <td align="center"><strong>ðŸŽ§ Audic Find</strong></td>
    </tr>
    <tr>
      <td><img src="Screenshots/sc_4.png" width="200" style="border-radius: 12px;"/></td>
      <td><img src="Screenshots/sc_5.png" width="200" style="border-radius: 12px;"/></td>
      <td><img src="Screenshots/sc_6.png" width="200" style="border-radius: 12px;"/></td>
    </tr>
  </table>
</div>

---

## ðŸŽ¯ Features

### ðŸŽ¶ Music & Playback
|  |  |
|---|---|
| ðŸš« **Zero Ads** | Listen endlessly â€” no interruptions, no subscriptions |
| ðŸŽ¬ **Audio & Video** | Seamlessly switch between audio-only and music video mode |
| ðŸŒ™ **Background Play** | Keep the music going while you use other apps or your screen is off |
| ðŸ“¥ **Offline Downloads** | Save songs to your device and listen anywhere, even offline |
| ðŸ“ **Local Files** | Play music stored directly on your phone |
| âž° **Crossfade & Canvas** | Smooth track transitions with beautiful playing animations |

### ðŸŽ¤ Lyrics Like Never Before
|  |  |
|---|---|
| â±ï¸ **Real-Time Synced Lyrics** | Every word in perfect timing â€” just like karaoke |
| ðŸ“¡ **Lyrics+ Providers** | LrcLib, KuGou, Unison, YouTube captions & more for best coverage |
| ðŸŒ **AI Translation** | Read lyrics translated into any language |
| ðŸŽ¨ **Lyric Animations** | Multiple styles, including word-by-word highlighting |

### ðŸ” Music Discovery
|                                  |  |
|----------------------------------|---|
| ðŸŽ§ **Audic Find**                | Tap to recognize any song playing around you â€” works like Shazam |
| ðŸ§  **Audic Brain**               | On-device AI learns your taste and auto-suggests tracks you'll love |
| ðŸŽµ **YouTube Music Integration** | Search, browse, and play from the world's largest music catalog |
| ðŸ§­ **Comprehensive Browsing**    | Explore charts, moods, genres, and podcasts |

### ðŸŽ§ Smart & Social
|  |  |
|---|---|
| ðŸ‘¥ **Listen Together** | Sync playback with friends in real time â€” a private listening party |
| ðŸ“‹ **Import from Spotify** | Bring your playlists and saved tracks over in seconds |
| ðŸŒ **Smart Sharing** | Share songs as short, clean links for cross-platform listening |
| ðŸ”” **Set as Ringtone** | Turn any song into your ringtone instantly |

### ðŸ”— Integrations
|  |  |
|---|---|
| ðŸ“» **Last.fm & Libre.fm** | Scrobble everything you play, sync your likes |
| ðŸ§  **ListenBrainz** | Open-source scrobbling alternative |
| ðŸ“£ **Cast to Devices** | Stream to speakers and Chromecast devices |

### ðŸŽ¨ Make It Yours
|  |  |
|---|---|
| ðŸŽ­ **Dynamic Themes** | Colors that adapt to your wallpaper (Material You) |
| ðŸ“ **Density Controls** | Compact or spacious UI â€” entirely your call |
| ðŸ–¼ï¸ **Mini Player Styles** | Classic, compact, or glass-like designs to choose from |
| ðŸ”„ **High Refresh Rate** | Smooth 90/120Hz animations on supported displays |

## ðŸ“¥ Download

Get the latest version of Audic Music:

> **ðŸ“± [Download the Latest APK](https://github.com/playpixelpro/audic/releases)**

Just install the APK and you're ready to go â€” no account needed, no setup required.

<details>
<summary>ðŸ”§ Building from source (for developers)</summary>

```bash
git clone https://github.com/playpixelpro/audic.git
cd audic
cp local.properties.template local.properties
# Edit local.properties with your SDK path
./gradlew assembleUniversalFossDebug
```

For detailed build instructions, see [SETUP.md](SETUP.md).
</details>

---

## ðŸ™ Special Thanks

Audic Music is made possible by these amazing open-source projects:

| Project | Why We Love It |
| :------ | :------------- |
| <a href="https://github.com/MetrolistGroup/Metrolist" target="_blank" rel="noopener noreferrer">Metrolist</a> & <a href="https://github.com/vivizzz007/vivi-music" target="_blank" rel="noopener noreferrer">Vivi Music</a> | Architecture & inspiration |
| <a href="https://github.com/koiverse/ArchiveTune" target="_blank" rel="noopener noreferrer">ArchiveTune</a> | Material You design ideas |
| <a href="https://better-lyrics.boidu.dev/" target="_blank" rel="noopener noreferrer">Better Lyrics</a> | Lyrics enhancements |
| <a href="https://github.com/maxrave-dev/SimpMusic" target="_blank" rel="noopener noreferrer">SimpMusic</a> | Lyrics implementation |
| <a href="https://github.com/aleksey-saenko/MusicRecognizer" target="_blank" rel="noopener noreferrer">Music Recognizer</a> | Audio recognition engine |
| <a href="https://github.com/a-edev/Flow" target="_blank" rel="noopener noreferrer">Flow</a> | AI queue generation |
| <a href="https://github.com/TeamNewPipe/NewPipeExtractor" target="_blank" rel="noopener noreferrer">NewPipe Extractor</a> | YouTube streaming |
| <a href="https://github.com/EchoMusicApp/Echo-Music" target="_blank" rel="noopener noreferrer">Echo Music</a> | Inspiration |

---

## ⚖️ Legal Disclaimer & Terms of Use

<div align="center">

| | |
|:---|---:|
| 🎵 | **This application is exclusively a** ***music player*** **— nothing more, nothing less.** |

</div>

---

### 🆓 1. Free and Open-Source **Music Player**

**Audic Music** is a fully free, open-source (FOSS) ***music player*** application built for educational and personal use. It is **not monetized** in any way, **no ads**, **no premium features**, **no subscriptions**, and **no hidden fees**. It is, at its heart, simply a ***music player***.

### 🎧 2. How This **Music Player** Works

**Audic Music** is a specialized ***music player*** that retrieves publicly available audio content and APIs from **YouTube** and **YouTube Music**, and plays them in a custom interface. The ad-free listening experience is similar to using a standard browser with an ad-blocking extension (such as uBlock Origin) — it does **not** modify or bypass content protections beyond that. This ***music player*** is designed purely for convenience and education.

### 🎨 3. Supporting Creators

We deeply respect the work of artists and content creators. Users are **strongly encouraged** to subscribe to <a href="https://www.youtube.com/premium" target="_blank" rel="noopener noreferrer"><strong>🎬 YouTube Premium</strong></a> to directly support the creators they listen to through this ***music player***. **Audic Music** is intended as a developer proof-of-concept, **not** as a way to reduce creator revenue. Please use this ***music player*** responsibly.

### ☁️ 4. No Hosted Content

**Audic Music** does **not** host, upload, or store any audio, video, or copyrighted media on its own servers. All content remains hosted on **Google/YouTube's servers** and is the property of its respective owners. This ***music player*** simply streams publicly accessible links — it is merely a ***music player***, not a content distribution platform.

### 📜 5. User Responsibility & Contact

This software is provided **"AS IS"**, without warranty of any kind. Users are solely responsible for ensuring their use of this ***music player*** complies with local copyright laws and the platform's **Terms of Service**. Because no media is hosted by us, we **cannot** process DMCA takedown requests for audio/video content.

> **For legal concerns regarding the open-source code itself, contact:**
> 📧 <a href="mailto:playpixelpro@gmail.com" target="_blank" rel="noopener noreferrer"><strong>playpixelpro@gmail.com</strong></a>

---

<div align="center">
  <p>Made with â¤ï¸ for music lovers everywhere</p>
  <p>Licensed under <a href="LICENSE">GPL-3.0</a></p>
  <br>
  <img src="assets/Audic-new.png" alt="Audic Music Logo" width="60"/>
</div>







