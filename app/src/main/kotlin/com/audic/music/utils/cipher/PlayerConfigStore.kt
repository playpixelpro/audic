...
object PlayerConfigStore {
    ...
    /** Last time a remote config fetch reached the server (200/304), or 0 if never. */
    fun getLastFetchTimeMs(): Long {
        return readMeta(FARADAY_SOURCE)?.second ?: 0L
    }

    /**
     * User-triggered manual refresh (Settings -> Extractor): fetch both remote tables now,
     * ignoring the TTL gate, and report success when the server was reached. Cooldown is reset so
     * a prior failure-triggered attempt never blocks an explicit user action.
     */
    suspend fun manualRefresh(): Boolean = withContext(Dispatchers.IO) {
        refreshMutex.withLock {
            lastForcedAttemptMs = 0L
            val faraday = fetchAndApply(FARADAY_SOURCE)
            val zemer = fetchAndApply(ZEMER_SOURCE)
            faraday.reachedServer || zemer.reachedServer
        }
    }
